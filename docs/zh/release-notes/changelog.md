...
- 新增会话级 `--thinking <effort>` 覆盖，适用于交互式、非交互式启动及恢复的会话。
- 通过继承当前模型与思考强度的有界、全新上下文验证 Agent 检查自主目标完成状态；该 Agent 会使用对抗性属性探针验证已交付行为，优先复查上次的具体缺口，并且仅允许一次禁用工具的裁决格式修复，随后安全拒绝完成请求。
