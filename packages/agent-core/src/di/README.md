# `@yaseenhq/agent-core` Dependency Injection Container
...
- **Testing** — `TestInstantiationService` (subpath
  `@yaseenhq/agent-core/di/test`) exposes direct `.get` / `.stub` so
...
Test files import from the subpath `@yaseenhq/agent-core/di/test`
(NOT the main package entry — keeps production bundles clean):

```ts
import { TestInstantiationService } from '@yaseenhq/agent-core/di/test';
...
├── test.ts                      ← subpath barrel (`@yaseenhq/agent-core/di/test`)
