/**
 * Tsugite-backed implementation of the loop `LLM` interface.
 *
 * Bridges the new `loop/llm.ts` contract onto
 * the tsugite `generate()` streaming API:
 *
 *   - tsugite's per-part `onMessagePart` is forwarded to loop per-delta
 *     callbacks (`onTextDelta`, `onThinkDelta`, `onToolCallDelta`).
 *   - loop per-block callbacks (`onTextPart`, `onThinkPart`) only fire
 *     after the tsugite stream drains, iterating over the merged
 *     `result.message.content`. Completed
 *     blocks land on the WAL seam, raw deltas never do.
 *   - tsugite's finish reasons are preserved as provider diagnostics. The loop
...
export type GenerateFn = typeof tsugiteGenerate;

export interface TsugiteLLMConfig {
  readonly provider: ChatProvider;
  readonly systemPrompt: string;
  readonly capability?: ModelCapability | undefined;
  /**
   * Optional override for the tsugite `generate()` entry point. Lets the
...
export class TsugiteLLM implements LLM {
  readonly systemPrompt: string;
  ...
  constructor(config: TsugiteLLMConfig) {
    this.provider = config.provider;
    this.modelName = config.provider.modelName;
    this.systemPrompt = config.systemPrompt;
    this.capability = config.capability;
    this.generate = config.generate ?? tsugiteGenerate;
  ...
  async chat(params: LLMChatParams): Promise<LLMChatResponse> {
    ...
    const callbacks = buildTsugiteCallbacks(params, markStreamOutput);
...
function buildTsugiteCallbacks(
