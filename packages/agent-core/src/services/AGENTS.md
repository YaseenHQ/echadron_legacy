...
`@yaseenhq/agent-core` (merged from the former `@yaseenhq/services`
...
from `index.ts` so importing `@yaseenhq/agent-core` runs the registration
...
2. **Consumers seed from `getSingletonServiceDescriptors()` directly**.
   Importing `@yaseenhq/agent-core` loads the package barrel, whose impl
