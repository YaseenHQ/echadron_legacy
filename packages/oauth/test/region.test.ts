...
describe('resolveKimiRegion', () => {
  // A test may need more than one marker dir (e.g. comparing home vars), so
  // track every handle rather than just the last one.
  let workDirs: TempDirHandle[] = [];

  afterEach(async () => {
    await Promise.all(workDirs.map(async (dir) => dir.cleanup()));
    workDirs = [];
  });
  ...
  it('honors the Echadron home vars when homeDir is not passed explicitly', async () => {
    const dir = await markerDir('global');
    expect(resolveKimiRegion({ env: { ECHADRON_HOME: dir } })).toBe('global');
    expect(resolveKimiRegion({ env: { ECHADRON_CODE_HOME: dir } })).toBe('global');
    expect(resolveKimiRegion({ env: { IMPERIUM_HOME: dir } })).toBe('global');
    // Kimi's own var stays a fallback for migrated installs.
    expect(resolveKimiRegion({ env: { KIMI_CODE_HOME: dir } })).toBe('global');
  });

  it('prefers ECHADRON_HOME over the legacy Kimi var', async () => {
    const echadron = await markerDir('global');
    const legacy = await markerDir('mainland-cn');
    expect(
      resolveKimiRegion({ env: { ECHADRON_HOME: echadron, KIMI_CODE_HOME: legacy } }),
    ).toBe('global');
  });
