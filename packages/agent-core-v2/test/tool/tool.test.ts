...
describe('SubagentToolInputSchema', () => {
  ...
  it('exposes the model choice parameter in the JSON schema', () => {
    ...
    // Open string rather than an enum: the two roles are still accepted, but a
    // caller can also name any configured [models] entry to route a subagent
    // at a model with the capabilities the task needs.
    ...
    expect(properties['model']?.description).toContain('id of any configured model');
  });
...
describe('Agent tool description', () => {
  ...
  it('renders the tool set for each subagent type', () => {
    ...
    // `coder` is what subagents are spawned as, so it must not advertise the
    // delegation tools — otherwise a subagent can spawn its own subagents and
    // fan-out becomes recursive.
    expect(description).toContain('Tools: Bash');
    expect(description).not.toContain('Tools: Agent, AgentSwarm, Bash');
  });
...
describe('Agent tool execution contract', () => {
  ...
  it('omits internal profiles from the tool description', () => {
    const catalog = allowlistCatalog(['coder', 'explore', 'verifier']);
    // Overlay an internal profile onto the catalog's list.
    ...
    const originalList = catalog.list();
    const overlay: ISessionAgentProfileCatalog = {
      ...catalog,
      list: () => [...originalList, internalProfile],
      get: (name) =>
        name === internalProfile.name
          ? internalProfile
          : catalog.get(name),
    };
    ctx = createTestAgent(sessionService(ISessionAgentProfileCatalog, overlay));
    const tool = ctx.toolsData().find((entry) => entry.name === 'Agent');
    expect(tool).toBeDefined();

    expect(tool!.description).not.toContain('Internal verifier');
    expect(tool!.description).not.toContain('- verifier:');
  });

  it('rejects an explicit attempt to launch an internal profile', async () => {
    const lifecycle = createAgentLifecycleStub();
    const catalog = allowlistCatalog(['coder', 'explore', 'verifier']);
    const internalProfile: AgentProfile = {
      name: 'verifier',
    ...
    const context = createAgentToolContext(
      lifecycle,
      sessionService(ISessionAgentProfileCatalog, overlay),
    );
...
describe('Agent tools', () => {
  ...
  describe('registered user tools', () => {
    ...
    it('routes registered user tools through tool.call request/response', async () => {
      ...
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        system: <system-prompt>
        tools: Agent, AgentSwarm, AskUserQuestion, Bash, Browser, CreateGoal, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, Lookup, Read, ReadDocument, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TodoList, UpdateGoal, Write
        messages:
    ...
    `);
      ...
      `);
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        tools: Agent, AgentSwarm, AskUserQuestion, Bash, Browser, CreateGoal, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, Read, ReadDocument, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TodoList, UpdateGoal, Write
        messages:
          <last>
