...
import { IAgentLoopService, effectiveMaxStepsPerTurn, type Turn } from '#/agent/loop/loop';
import {
  LOOP_MAX_STEPS_LANDING_KIND,
  LOOP_MAX_STEPS_LANDING_PROMPT,
} from '#/agent/loop/loopContinuation';
...
describe('effectiveMaxStepsPerTurn', () => {
  it('returns undefined when neither global nor profile sets a positive ceiling', () => {
    expect(effectiveMaxStepsPerTurn(undefined, undefined)).toBeUndefined();
    expect(effectiveMaxStepsPerTurn(0, 0)).toBeUndefined();
    expect(effectiveMaxStepsPerTurn(undefined, 0)).toBeUndefined();
    expect(effectiveMaxStepsPerTurn(0, undefined)).toBeUndefined();
  });

  it('uses the global ceiling alone when no profile ceiling is set', () => {
    expect(effectiveMaxStepsPerTurn(20, undefined)).toBe(20);
    expect(effectiveMaxStepsPerTurn(20, 0)).toBe(20);
  });

  it('uses the profile ceiling alone when no global ceiling is set', () => {
    expect(effectiveMaxStepsPerTurn(undefined, 10)).toBe(10);
    expect(effectiveMaxStepsPerTurn(0, 10)).toBe(10);
  });

  it('takes the smaller positive ceiling when both are set', () => {
    expect(effectiveMaxStepsPerTurn(50, 10)).toBe(10);
    expect(effectiveMaxStepsPerTurn(10, 50)).toBe(10);
    expect(effectiveMaxStepsPerTurn(5, 5)).toBe(5);
  });
});

describe('profile step ceiling', () => {
  ...
  const echoTool: ExecutableTool = {
    name: 'Echo',
    description: 'Echo a marker so the loop continues.',
    parameters: { type: 'object', properties: {}, required: [], additionalProperties: false },
    resolveExecution: () => ({
      approvalRule: 'Echo',
      execute: async () => ({ output: 'echoed' }),
    }),
  };

  async function driveWithCeiling(profileCeiling: number, globalCeiling?: number): Promise<{
    result: Turn['result'] extends Promise<infer R> ? R : never;
    ctx: TestAgentContext;
    echoCalls: number;
  }> {
    const ctx = createTestAgent(permissionModeServices('yolo'));
    const profile = ctx.get(IAgentProfileService);
    await profile.applyProfile({
      name: 'test-step-ceiling',
      description: 'test profile carrying a per-turn step ceiling',
      maxStepsPerTurn: profileCeiling,
      systemPrompt: () => 'test-step-ceiling',
    });
    profile.update({ activeToolNames: ['Echo'] });
    let echoCalls = 0;
    ...
    if (globalCeiling !== undefined) {
      (ctx.kimiConfig as Record<string, unknown>)['loopControl'] = { maxStepsPerTurn: globalCeiling };
    }
    for (let index = 0; index < profileCeiling; index += 1) {
      ctx.mockNextResponse({ type: 'text', text: `step ${String(index + 1)}` }, echoCall);
    }
    ctx.mockNextResponse({ type: 'text', text: 'landing wrap-up' });
    const loop = ctx.get(IAgentLoopService);
    await ctx.rpc.prompt({ input: [{ type: 'text', text: 'go' }] });
    const turn = (loop as unknown as { activeTurnJob?: { turn: Turn } }).activeTurnJob?.turn;
    expect(turn).toBeDefined();
    const result = await turn!.result;
    return { result, ctx, echoCalls };
  }

  it('lands a tool-free wrap-up after the bound profile ceiling instead of failing the turn', async () => {
    const { result, ctx, echoCalls } = await driveWithCeiling(2, undefined);
    try {
      ...
      expect(
        history.some(
          (message) =>
            ...
            message.origin.name === LOOP_MAX_STEPS_LANDING_KIND &&
            message.content.some(
              (part) => part.type === 'text' && part.text.includes(LOOP_MAX_STEPS_LANDING_PROMPT.slice(0, 40)),
            ),
        ),
      ).toBe(true);
    } finally {
  ...
  it('takes the smaller global ceiling over a looser bound profile ceiling and still lands', async () => {
    const { result, ctx } = await driveWithCeiling(50, 2);
    try {
      expect(result.type).toBe('completed');
      expect(result.steps).toBe(3);
    } finally {
  ...
  it('keeps tools available on the next turn after a max-steps landing', async () => {
    const { ctx } = await driveWithCeiling(2, undefined);
    try {
      ctx.mockNextResponse({ type: 'text', text: 'second turn' });
      await ctx.rpc.prompt({ input: [{ type: 'text', text: 'again' }] });
      await ctx.untilTurnEnd();
      const requests = ctx.allEvents.filter(
        (event) => event.type === '[wire]' && event.event === 'llm.request',
      );
      const firstWork = requests[0];
      const nextTurn = requests.at(-1);
      expect(nextTurn?.args).toMatchObject({
        toolsHash: (firstWork?.args as { toolsHash?: string } | undefined)?.toolsHash,
      });
    } finally {
      await ctx.dispose();
    }
