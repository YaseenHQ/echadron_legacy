/**
 * `tsugiteConfig` bridge tests — `TsugiteConfigService`, the two-way sync
 * between `IConfigService` (persistence) and tsugite's in-memory
 ...
 *    `ready` themselves;
 *  - tsugite → config: registry mutations (`set` / `setDefaultModel` / ...)
 ...
 *  - config → tsugite: section writes (`config.set` / `config.replace`) land
 *    in the registries;
...
import { ModelService } from '#/tsugite/model/modelService';
import { type ProviderConfig } from '#/tsugite/provider/provider';
import { ProviderService } from '#/tsugite/provider/providerService';

import { StubConfigService } from '../../tsugite/stubs';
import { TsugiteConfigService } from '#/app/tsugiteConfig/tsugiteConfigService';
...
function stubLogService(): ILogService & { warnings: Array<{ message: string; payload?: LogPayload }> } {
  ...
  return {
    ...
    debug: () => {},
    child: () => {
      throw new Error('child loggers are not used by TsugiteConfigService');
    },
...
interface BridgeFixture {
  ...
  readonly bridge: TsugiteConfigService;
}
...
async function createBridge(sections: Record<string, unknown> = {}): Promise<BridgeFixture> {
  ...
  const bridge = new TsugiteConfigService(config, providers, models, log);
  await bridge.ready;
...
describe('TsugiteConfigService startup hydration', () => {
...
describe('TsugiteConfigService tsugite → config persistence', () => {
...
describe('TsugiteConfigService awaited-mutation semantics', () => {
  ...
  it('logs and resolves after the retry budget is spent, and the chain stays alive', async () => {
    ...
    try {
      ...
      expect(log.warnings[0]?.message).toBe('tsugite config persist failed');
...
describe('TsugiteConfigService config → tsugite sync', () => {
...
describe('TsugiteConfigService loop termination', () => {
  it('a tsugite-originated persist does not echo back as a tsugite change', async () => {
    const { config, providers, bridge } = await createBridge(seededSections);
    try {
      const replaceSpy = vi.spyOn(config, 'replace');
      const events: string[] = [];
...
describe('TsugiteConfigService default-provider deletion', () => {
...
describe('TsugiteConfigService env-pinned default pointer', () => {
  /**
  ...
  it('re-asserts the pinned effective default model into the registry after a registry-originated write', async () => {
    ...
    const providers = new ProviderService();
    const models = new ModelService();
    const bridge = new TsugiteConfigService(config, providers, models, stubLogService());
    await bridge.ready;
    try {
