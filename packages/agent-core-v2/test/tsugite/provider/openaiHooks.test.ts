/**
 * `tsugite/provider` compositor probes (probe 5) — `composeOpenAIChatHooks`
...
import {
  ...
  traitEndpoint,
  traitProvides,
} from '#/tsugite/provider/bases/openai/openaiHooks';
...
describe('traitEndpoint / firstProcessEnv', () => {
  const ENV_A = 'TSUGITE_TEST_ENV_A';
  const ENV_B = 'TSUGITE_TEST_ENV_B';
