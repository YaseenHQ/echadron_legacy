/**
 * `tsugite/model` ModelCatalog tests — Model assembly, caching, and
...
import type { ChatProvider } from '#/tsugite/contract/provider';
...
import '#/tsugite/provider/protocolAdapterRegistry';
...
import { IModelService, type ModelRecord, type ModelsSection } from '#/tsugite/model/model';
...
function createHost(
  ...
  // Tsugite's registries are pure in-memory stores now (persistence lives in
  // the app/tsugiteConfig bridge): seed them from the fixture sections.
...
describe('ModelCatalog setDefaultModel', () => {
  it('moves the registry default pointer and returns the wire model', async () => {
    ...
    try {
      ...
      // the app/tsugiteConfig bridge's job.
