 ...
 * and recovers the last turn outcome from the persisted TurnModel on restore;
 * it otherwise holds only derived state, so it can be discarded and rebuilt
...
export class AgentActivityView extends Disposable implements IAgentActivityView {
  ...
  constructor(
    ...
    this._register(
      this.wire.hooks.onDidRestore.register('activity-view', async (_ctx, next) => {
