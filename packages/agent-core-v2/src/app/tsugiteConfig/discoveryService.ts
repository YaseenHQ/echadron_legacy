/**
 * `tsugiteConfig` domain (L3) — `IProviderDiscoveryService` implementation.
 *
 * Owns the all-provider model refresh: delegates to the shared
 * `@yaseenhq/echadron-oauth` orchestrator (managed OAuth + open
 ...
 * persistence bridge then syncs them into tsugite's in-memory registries),
...
import { getProviderDefinition } from '#/tsugite/provider/providerDefinition';
...
export class ProviderDiscoveryService implements IProviderDiscoveryService {
  ...
  private async applyRefreshPatch(
    ...
    // new records into the tsugite registries — no reader can observe a
