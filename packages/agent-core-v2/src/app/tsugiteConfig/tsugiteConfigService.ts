/**
 * `tsugiteConfig` domain (L3) — `ITsugiteConfigService` implementation.
 *
 * The two-way persistence bridge between `IConfigService` and tsugite's
 * in-memory provider/model registries. See `tsugiteConfig.ts` for the
 ...
 *
 *  - config → tsugite: the registries' writes are silent when the value is
 *    equal, so a config-originated push never echoes back as a persist.
 *  - tsugite → config: the persist handlers skip the write when the config
...
import { IProviderService, type ProvidersSection } from '#/tsugite/provider/provider';

import { ITsugiteConfigService } from './tsugiteConfig';
import {
...
export class TsugiteConfigService extends Disposable implements ITsugiteConfigService {
  ...
  constructor(
    ...
    void this.ready.catch((error) => {
      this.log.warn('tsugite config bridge initialization failed', {
  ...
  private logPersistFailure(error: unknown): void {
    this.log.warn('tsugite config persist failed', { error: describeUnknownError(error) });
  }
}
