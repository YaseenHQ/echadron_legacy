/**
 * `tsugiteConfig` domain (L3) — the third-party models.dev directory: its
 ...
 * lives here in the app layer, NOT in tsugite — tsugite's type surface stays
 * limited to the engine's own built-in vocabulary. The translation boundary
 * is this file: its output (`ModelsDevModel`) is already expressed in tsugite
 ...
 *
 * Ported from `@yaseenhq/tsugite`'s `catalog.ts` so the engine stays
 * self-contained; the two copies must be kept in lockstep while the v1
 * lineage (agent-core / node-sdk / CLI) still consumes the tsugite one.
 */

import type { ModelCapability } from '#/tsugite/contract/capability';
import type { ProviderType } from '#/tsugite/provider/provider';

import { wireHasProtocolThinkingDisable } from '#/tsugite/model/thinking';
 ...
 * adapters expose an OpenAI-family HTTP boundary and can use tsugite's OpenAI
...
export function modelsDevProviderModels(entry: ModelsDevProviderEntry): ModelsDevModel[] {
  ...
  return result.map((model) => {
    ...
    // (tsugite's `wireHasProtocolThinkingDisable` verdict), so marking those
