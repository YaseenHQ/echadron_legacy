/**
 * `tsugiteConfig` domain (L3) — `IProviderDiscoveryService`: remote model
 ...
 * registries) through the shared `@yaseenhq/echadron-oauth` orchestrator,
 * applies the result to tsugite's in-memory registries (the persistence
 * bridge writes it back to config), and publishes
 * `event.model_catalog.changed` on change. This is a WRITE path (external
 * world → tsugite → config), deliberately separate from the read-only
