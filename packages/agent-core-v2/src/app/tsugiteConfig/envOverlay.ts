/**
 * `tsugiteConfig` domain (L3) — `KIMI_MODEL_*` effective-config overlay.
 *
 * When `KIMI_MODEL_NAME` is set, synthesizes one model id (bound to the
 * reserved `__kimi_env__` provider whose schema tsugite owns) from the
 ...
 * imports tsugite semantics, and so the overlay takes effect even when the
 * tsugite registry services are never instantiated.
 *
 * The env provider's default `baseUrl` is resolved through tsugite's
...
import { CONFIG_INVALID_ERROR_CODE } from '#/tsugite/contract/errors';
import { resolveProviderEndpoint } from '#/tsugite/provider/providerDefinition';
