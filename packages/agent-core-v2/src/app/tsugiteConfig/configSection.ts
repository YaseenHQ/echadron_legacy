/**
 * `tsugiteConfig` domain (L3) — config-section declarations for tsugite.
 *
 * The persistence wrapper for tsugite's provider/model registries and the
 * thinking / model-catalog / secondary-model preferences: declares every
 * tsugite-owned section constant and its zod schema, plus the env bindings /
 * write-path strips and the snake_case ↔ camelCase TOML transforms. Where
 * tsugite owns a pure type (`providers` / `models` / `thinking`), the schema
 ...
 * tsugite-side type — theirs derive from the local schemas. Self-registered
 * at module load via `registerConfigSection`, so the `config` domain never
 * imports tsugite types.
 *
 ...
 * tsugite's provider-definition registry, which is what allows external
...
import type { ModelOverride, ModelRecord, ModelsSection } from '#/tsugite/model/model';
import type { ThinkingConfig } from '#/tsugite/model/thinking';
import type { OAuthRef, ProviderConfig, ProvidersSection } from '#/tsugite/provider/provider';
import { ProtocolSchema } from '#/tsugite/protocol/protocol';

// ---------------------------------------------------------------------------
// `providers` — tsugite's provider registry (types: `tsugite/provider/provider`)
// ---------------------------------------------------------------------------
...
// Compile-time pins: the schemas must stay in lockstep with tsugite's
...
// ---------------------------------------------------------------------------
// `models` — tsugite's model registry (types: `tsugite/model/model`)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// `thinking` — thinking defaults (type: `tsugite/model/thinking`)
// ---------------------------------------------------------------------------
...
// Compile-time pin: the schema must stay in lockstep with tsugite's
...
// thinking effort. No tsugite-side type — derived from the schema.
...
// ---------------------------------------------------------------------------
// `modelCatalog` — provider-model catalog auto-refresh cadence (no tsugite type)
// ---------------------------------------------------------------------------
