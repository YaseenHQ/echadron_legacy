/**
 * `tsugiteConfig` domain (L3) — `IModelsDevImportService` implementation.
 *
 ...
 * `@yaseenhq/echadron-oauth` primitives — the exact remove-then-apply
 ...
 * passes (drop, then re-add onto clean slots). The tsugite persistence
...
import { type ProviderConfig, type ProvidersSection } from '#/tsugite/provider/provider';
...
export class ModelsDevImportService implements IModelsDevImportService {
  ...
  constructor(
    ...
    @ITsugiteConfigService private readonly tsugiteConfig: ITsugiteConfigService,
  ...
  /**
   * Await the config layer AND the tsugite persistence bridge. The bridge
   * subscribes to section changes after the initial hydration; awaiting it
   * guarantees a write below reaches the tsugite registries (and the
  ...
  private async readyConfig(): Promise<IConfigService> {
    ...
    await this.tsugiteConfig.ready;
    return this.config;
  }
