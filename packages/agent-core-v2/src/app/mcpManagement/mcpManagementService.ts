...
export class McpManagementService extends Disposable implements IMcpManagementService {
  ...
  private async withProbe<T>(
    ...
    // Upstream binds a stdio probe to the workspace instance that contains the
    // cwd, so it can run on that workspace's runtime. There is no
    // workspace-instance layer here yet, so a probe always runs locally --
    // which is what every stdio server does in this build anyway.
