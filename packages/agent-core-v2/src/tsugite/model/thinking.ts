/**
 * `tsugite/model` domain (L2) — the single authority on thinking semantics.
 *
 * Three kinds of knowledge live here, and nowhere else:
 *
 *  1. The `thinking` config-section type (`[thinking]`: enabled / effort /
 *     keep, plus the env-only `forcedEffort` field). Tsugite owns only the
 ...
 *     persistence wrapper (`app/tsugiteConfig/configSection`).
...
import type { IProtocolAdapterRegistry, Protocol } from '#/tsugite/protocol/protocol';
...
// ---------------------------------------------------------------------------
// `thinking` config-section type (constant + schema live in `app/tsugiteConfig`)
// ---------------------------------------------------------------------------
 ...
 * `always_thinking` marker exists to capture. Tsugite owns this verdict (the
