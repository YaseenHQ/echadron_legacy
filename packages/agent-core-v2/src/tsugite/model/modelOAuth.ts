/**
 * `tsugite/model` domain (L2) — the OAuth token port.
 *
 * Tsugite needs OAuth tokens at model-assembly time: probing the cached
 * credential state (catalog listings) and building the refreshable request
 * auth closure. The port is owned here so tsugite stays free of the
 * `app/auth` service; the implementation lives in the upper layer
 * (`app/tsugiteConfig/oauthTokenAdapter.ts`), which delegates to
