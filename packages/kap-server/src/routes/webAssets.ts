...
async function assertWebAssets(assetsDir: string): Promise<void> {
  try {
    ...
    throw new Error(
      `Echadron web assets were not found at ${assetsDir}. Run the package build before starting the server.`,
    );
  }
}
...
async function serveWebAsset(
  ...
  const requestUrl = new URL(req.url, 'http://echadron-web.local');
