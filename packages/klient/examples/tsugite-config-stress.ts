/**
 * Stress check for the tsugite ⇄ tsugiteConfig read/write path, against an
 * in-process engine (memory transport) bootstrapped on a throwaway home.
 *
 * The contract under pressure: an awaited tsugite mutation resolves only after
 * the write has been persisted by the tsugiteConfig bridge (with retries on
 * transient disk failures), and config-originated writes land in the tsugite
 * registries synchronously. Every phase asserts read-after-write visibility
 * through BOTH facades — `tsugite.*` (registry view) and `config.get` (the
 ...
 *   6. config → tsugite direction (config.replace must be visible to the
 ...
 *
 *   pnpm -C packages/klient stress:tsugite-config
 *
...
import { ITsugiteConfigService } from '@yaseenhq/agent-core-v2/app/tsugiteConfig/tsugiteConfig';
import { type Klient } from '@yaseenhq/klient';
...
async function main(): Promise<void> {
  const homeDir = await mkdtemp(join(tmpdir(), 'klient-tsugite-stress-'));
  ...
  try {
    ...
    const tsugite = klient.global.tsugite;
    ...
    await phase('sequential provider add read-after-write', 30, async () => {
      for (let i = 0; i < 30; i += 1) {
        ...
        await tsugite.addProvider(name, apiKeyProvider(`sk-seq-${String(i)}`));
        ...
        const got = await tsugite.getProvider(name);
    ...
    await phase('same-name add/remove flip-flop', 30, async () => {
      for (let i = 0; i < 15; i += 1) {
        await tsugite.addProvider('flip', apiKeyProvider(`sk-flip-${String(i)}`));
        ...
        await tsugite.removeProvider('flip');
    ...
    await phase('default-model churn', 22, async () => {
      await tsugite.addProvider(anonymousModel('churn_a'));
      await tsugite.addProvider(anonymousModel('churn_b'));
      for (let i = 0; i < 20; i += 1) {
        const id = i % 2 === 0 ? 'churn_a' : 'churn_b';
        await tsugite.setDefaultModel(id);
    ...
    await phase('concurrent burst adds', 40, async () => {
      await Promise.all(
        Array.from({ length: 40 }, (_, i) =>
          tsugite.addProvider(`burst_${String(i)}`, apiKeyProvider(`sk-burst-${String(i)}`)),
        ),
      );
      const providers = await config.get<ProvidersSectionView>('providers');
    ...
    await phase('concurrent mixed sections', 41, async () => {
      await Promise.all([
        ...Array.from({ length: 30 }, (_, i) => tsugite.removeProvider(`seq_${String(i)}`)),
        ...Array.from({ length: 10 }, (_, i) => tsugite.addProvider(anonymousModel(`mix_${String(i)}`))),
        tsugite.setDefaultModel('churn_b'),
      ]);
      const providers = await config.get<ProvidersSectionView>('providers');
    ...
    // 6) config → tsugite: a config-originated write must be visible to the
    ...
    await phase('config-originated replace visible to registry', 4, async () => {
      ...
      const got = await tsugite.getProvider('cfg_only');
      assert(got !== undefined, 'config.replace(providers) visible to tsugite.getProvider');
      ...
      const modelIds = (await tsugite.listModels()).map((m) => m.model);
      assert(modelIds.includes('cfg_model'), 'config.replace(models) visible to tsugite.listModels');
      ...
      await tsugite.removeProvider('cfg_only');
      assert(
  ...
  await phase('restart durability', 1, async () => {
    ...
    try {
      ...
      await app2.accessor.get(ITsugiteConfigService).ready;
      ...
      const providerIds = new Set((await klient2.global.tsugite.listProviders()).map((p) => p.id));
  ...
  console.log('tsugite-config stress: OK');
}
