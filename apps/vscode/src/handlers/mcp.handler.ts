...
import type { McpServerConfig as SdkMcpServerConfig, McpTestResult } from "@yaseenhq/echadron-sdk";
...
export const mcpHandlers: Record<string, Handler<any, any>> = {
    ...
    await vscode.window.withProgress(
      {
        ...
        title: `Echadron: Authenticating "${name}"...`,
        cancellable: false,
      },
      async () => {
        try {
          ...
          await vscode.window.showInformationMessage(`Echadron: OAuth completed for "${name}"`);
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          await vscode.window.showErrorMessage(`Echadron: OAuth failed for "${name}": ${message}`);
          throw error;
        }
      },
    );
  ...
  [Methods.ResetAuthMCP]: async ({ name }: NameParams, ctx) => {
    await vscode.window.withProgress(
      {
        location: vscode.ProgressLocation.Notification,
        title: `Echadron: Resetting auth for "${name}"...`,
        cancellable: false,
      },
      async () => {
        try {
          await ctx.harness.resetMcpServerAuth(name);
          await vscode.window.showInformationMessage(`Echadron: Auth reset for "${name}"`);
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          await vscode.window.showErrorMessage(`Echadron: Reset auth failed for "${name}": ${message}`);
          throw error;
        }
      },
    );
  ...
  [Methods.TestMCP]: async ({ name }: NameParams, ctx): Promise<MCPTestResult> => {
    void vscode.window.showInformationMessage(`Echadron: Testing MCP server "${name}"...`);
