# apps/echadron Development Guide

This file only contains rules local to `apps/echadron`. For cross-repo rules, see the root `AGENTS.md`.
...
`apps/echadron` is the terminal UI / CLI app. The entry chain is:
...
- `apps/echadron` may only use core capabilities through `@yaseenhq/echadron-sdk`. Do not import `@yaseenhq/agent-core` directly in app code.
