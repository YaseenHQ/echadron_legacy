/**
 * `echadron server kill` — deprecated; only stops a server started by an old
 * (pre-`echadron web`, i.e. before 0.28.0) build.
 *
...
/**
 * The first release whose servers run in the foreground (`echadron web`) and
...
/** Deprecation notice printed on every `echadron server kill` run. */
export const DEPRECATED_KILL_NOTICE =
  '`echadron server kill` is deprecated: it only stops servers started by a version before 0.28.0. Servers started by `echadron web` run in the foreground — stop them with Ctrl+C.\n';
...
export function registerLegacyKillCommand(server: Command): void {
  server
    .command('kill')
    .description(
      'Deprecated — stop a server started by a version before 0.28.0 (recorded in the legacy server lock). Servers started by `echadron web` run in the foreground — stop them with Ctrl+C.',
    )
...
export async function handleLegacyKillCommand(deps: LegacyKillDeps): Promise<void> {
  ...
  if (lock === undefined) {
    deps.stdout.write('No running legacy Echadron server.\n');
    return;
  }
  ...
  await deps.removeLock().catch(() => {});
  deps.stdout.write(`Legacy Echadron server (pid ${String(lock.pid)}) ${outcome}.\n`);
}
...
async function killLegacyServer(
  ...
  throw new Error(
    `Failed to stop legacy Echadron server (pid ${String(pid)}); insufficient permissions?`,
  );
}
