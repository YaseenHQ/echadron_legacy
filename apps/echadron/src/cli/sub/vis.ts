/**
 * `echadron vis` sub-command.
 *
 * CLI glue only: resolves the Echadron home, starts the in-process session
...
export async function handleVis(deps: VisDeps, opts: VisOptions): Promise<void> {
  ...
  deps.stderr.write(`Failed to start echadron vis: ${msg}\n`);
    return deps.exit(1);
  }
  ...
  deps.stdout.write(`echadron vis is running at ${server.url}\n`);
  ...
  return {
    ...
    startVisServer:
      ...
      (async (opts) => {
        ...
        const { startVisServer } = await import('@yaseenhq/vis-server/start');
