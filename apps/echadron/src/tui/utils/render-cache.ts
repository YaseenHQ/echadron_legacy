 ...
 * `ECHADRON_TUI_NO_RENDER_CACHE=1` as an escape hatch (and to let benchmarks
 * compare cached vs. uncached runs in the same process).
 */

let enabled =
  (process.env['ECHADRON_TUI_NO_RENDER_CACHE'] ?? process.env['KIMI_TUI_NO_RENDER_CACHE']) !== '1';
