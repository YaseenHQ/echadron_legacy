import type { PluginInfo, PluginSummary } from '@yaseenhq/echadron-sdk';
...
export function buildPluginsInfoLines(input: PluginsInfoPanelInput): readonly string[] {
  ...
  const trustLine = (() => {
    ...
    if (label === 'official') {
      return `${muted('Trust:')}  ${success(OFFICIAL_BADGE)} ${muted('(host-maintained)')}`;
    }
    if (label === 'curated') {
      return `${muted('Trust:')}  ${primary(CURATED_BADGE)} ${muted('(curated, upstream-maintained)')}`;
    }
