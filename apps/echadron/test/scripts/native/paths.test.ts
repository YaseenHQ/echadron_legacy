...
describe('executableName', () => {
  it('returns echadron.exe on win32', () => {
    expect(executableName('win32')).toBe('echadron.exe');
  });

  it('returns echadron on other platforms', () => {
    expect(executableName('darwin')).toBe('echadron');
    expect(executableName('linux')).toBe('echadron');
  });
});
...
describe('path helpers', () => {
  ...
  it('returns absolute bin path with executable name', () => {
    expect(nativeBinPath('darwin-arm64', 'darwin')).toBe(
      p('dist-native/bin/darwin-arm64/echadron'),
    );
    expect(nativeBinPath('win32-x64', 'win32')).toBe(
      p('dist-native/bin/win32-x64/echadron.exe'),
    );
  });

  it('returns intermediate artifact paths', () => {
    expect(nativeJsBundlePath()).toBe(p('dist-native/intermediates/main.cjs'));
    expect(nativeBlobPath()).toBe(p('dist-native/intermediates/echadron.blob'));
