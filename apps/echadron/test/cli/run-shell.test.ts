...
import type { createKimiDeviceId as createKimiDeviceIdFn } from '@yaseenhq/echadron-oauth';
...
vi.mock('@yaseenhq/echadron-sdk', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@yaseenhq/echadron-sdk')>();
...
vi.mock('@yaseenhq/echadron-oauth', async () => {
  const actual = await vi.importActual<typeof import('@yaseenhq/echadron-oauth')>(
...
vi.mock('@yaseenhq/echadron-telemetry', () => ({
...
describe('runShell', () => {
  beforeEach(() => {
    // Keep the v1 harness assertions explicit now that v2 is the production
    // default. The first test below verifies the default v2 route.
  ...
  it('builds the v2 harness by default', async () => {
    stubTuiStartup();
    await withEnv({ ECHADRON_LEGACY_FLAG: undefined }, async () => {
  ...
  it('keeps the v1 harness when the legacy switch is set', async () => {
    stubTuiStartup();
    await withEnv({ ECHADRON_LEGACY_FLAG: '1' }, async () => {
      await runShell(minimalCliOptions, '1.2.3-test');
    });
