...
changesets already pre-generates `apps/echadron/CHANGELOG.md` inside the release PR. Extract the new version block from the diff:

```bash
gh api repos/MoonshotAI/kimi-code/pulls/<RELEASE>/files \
  --jq '.[] | select(.filename=="apps/echadron/CHANGELOG.md") | .patch'
```
